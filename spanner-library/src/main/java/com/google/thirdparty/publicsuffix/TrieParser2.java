...
class TrieParser2 {
   ...
   */
  static ImmutableMap<String, PublicSuffixType2> parseTrie(CharSequence encoded) {
    ImmutableMap.Builder<String, PublicSuffixType2> builder = ImmutableMap.builder();
  ...
  private static int doParseTrieToBuilder(
      ...
      ImmutableMap.Builder<String, PublicSuffixType2> builder) {
    ...
    if (c == '!' || c == '?' || c == ':' || c == ',') {
      ...
      if (domain.length() > 0) {
        builder.put(domain, PublicSuffixType2.fromCode(c));
      }
    }
    idx++;
